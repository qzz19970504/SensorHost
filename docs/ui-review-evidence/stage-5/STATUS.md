...
- 发布包：`tools/package_host.ps1`（未用 SkipTests）→
  `dist/host/STM32SensorHost-0.1.0-win64.exe`；**已用最终代码重建**：
  64,519,124 B，sha256 `a3c38c4df52e9e15c2c66ddce652bf447869f85a3376aa7e51832d2c5280ef36`；
  build-venv 内 pytest 通过、打包 `--smoke-test` 通过。
...
- 实机多页验证：`native-cdc-diagnostics.png`、`native-cdc-console.png` 确认诊断页
  搜索/分区跳转/错误摘要与 Console 时间戳/未读标记在真实数据下正常。
