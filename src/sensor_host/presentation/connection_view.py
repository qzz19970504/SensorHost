...
class WifiConnectionPanel(QFrame):
    ...
    def __init__(self) -> None:
        ...
        self.form_scroll_area.setHorizontalScrollBarPolicy(
        ...
        iface_label = QLabel("PHONE HOTSPOT INTERFACE")
        iface_label.setWordWrap(True)
        layout.addWidget(iface_label)
        self.interface_combo = IntegratedComboBox()
        self.interface_combo.setAccessibleName("Phone hotspot interface")
        iface_label.setBuddy(self.interface_combo)
        self.interface_combo.currentIndexChanged.connect(self._copy_selected_ipv4)
        layout.addWidget(self.interface_combo)
        ip_label = QLabel("PC IPv4 PRESET IN ESP FIRMWARE")
        ...
        self.expected_ipv4_edit.setAccessibleName("PC IPv4 preset in ESP firmware")
        ip_label.setBuddy(self.expected_ipv4_edit)
        layout.addWidget(self.expected_ipv4_edit)
        tcp_label = QLabel("TCP LISTEN PORT")
        tcp_label.setWordWrap(True)
        layout.addWidget(tcp_label)
        self.tcp_port_spin = QSpinBox()
        self.tcp_port_spin.setRange(1, 65_535)
        self.tcp_port_spin.setValue(DEFAULT_TCP_PORT)
        self.tcp_port_spin.setAccessibleName("TCP listen port")
        tcp_label.setBuddy(self.tcp_port_spin)
        layout.addWidget(self.tcp_port_spin)
        udp_label = QLabel("UDP WAKE PORT")
        udp_label.setWordWrap(True)
        ...
        self.udp_port_spin.setAccessibleName("UDP wake port")
        udp_label.setBuddy(self.udp_port_spin)
        layout.addWidget(self.udp_port_spin)
        targets_label = QLabel("OPTIONAL ESP IPv4 TARGETS")
        ...
        self.wake_targets_edit.setAccessibleName("Optional ESP IPv4 targets")
        targets_label.setBuddy(self.wake_targets_edit)
